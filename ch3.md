...
还有不同的策略来影响甚至决定SSTables如何被压缩和合并的顺序和时间。最常见的是大小分级和分层压缩。 LevelDB和RocksDB使用分层压缩（因此命名LevelDB），HBase使用大小分级，Cassandra同时支持这两种压缩【16】。在大小分级的压缩中，较新和较小的SSTables被连续合并到较旧和较大的SSTable中。在分层压缩中，键的范围被拆分成多个更小的SSTables，而较旧的数据被移动到单独的“层级”，这样压缩可以逐步进行并节省磁盘空间。
